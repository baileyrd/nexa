...
Controls: `1` skeleton report, `2` morph inspection, `M` select the next exported morph target, `3` animation selection, `Space` play/pause, `[` / `]` scrub, arrow keys orbit, `G` toggle eye/head gaze, `V` trigger a viseme hook, `R` reset. `Esc` closes.
