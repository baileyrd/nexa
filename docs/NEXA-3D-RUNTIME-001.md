...
Manifest validation additionally requires glTF `-Z` forward convention, verifies the canonical armature, head, eye, and jaw names against the exported node inventory, and verifies every expression/viseme mapping against a stable exported morph-target ID.
