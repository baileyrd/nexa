...
Status: Approved for R2 implementation
Date: 2026-08-26
...
The following documents are approved as R2 governing requirements where they apply to the R2 text-first path:
...
### 3.9 UI framework and accessibility

`eframe`/`egui` is approved for the R2 application subject to the bounded spike in ADR-0068.

R2 must support:

- keyboard operation of the core learner flow where the framework/platform permits;
- visible focus/status/error states;
- text scaling/layout resilience sufficient for the supported application;
- no instructional dependency on speech or animation.
...
## 5. R2 acceptance scenario adopted

The primary end-to-end scenario in the testing/acceptance supplement is the R2 exit authority, narrowed to the local-model text path:

1. launch the actual desktop app on Windows x86_64;
2. open/create the concrete SQLite store;
3. load the governed TCP lesson package;
4. submit text through the actual learner UI;
5. load/derive learning and pedagogy context;
6. retrieve governed knowledge from the concrete durable path;
7. invoke the concrete local llama.cpp adapter;
8. admit and quality-check the response;
9. present it in the actual UI;
10. complete assessment/practice;
11. atomically commit evidence/mastery/progress;
12. exit and relaunch;
13. resume exact accepted progress without duplication/loss;
14. complete the bounded lesson acceptance outcome;
15. verify content-safe operational correlation.

Scripted providers and in-memory repositories may support lower-level tests but cannot satisfy this scenario.

## 6. Performance posture for R2

R2 performance work is measurement-first.
...
## 7. R1 items explicitly not blocking R2

The following remain valid future requirements but do not block R2 start:
