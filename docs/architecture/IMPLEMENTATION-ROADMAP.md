...
- [x] provider-neutral synchronous model-input counting with content-free replay evidence and a deterministic scripted tokenizer; concrete tokenizers and capacity integration remain deferred
...
ADR-0036 adds a synchronous exact-model token-counting port, strict content-free replay evidence, and deterministic scripted testing infrastructure. ADR-0022 and ADR-0027 conservative UTF-8 byte capacity accounting remains authoritative; concrete tokenizers and integration into selection, request validation, invocation, admission, or routing remain unimplemented. No inference, provider, network, endpoint, credential, fallback, retry, repair, semantic-validation, privacy-policy, partial-truncation, async/streaming, telemetry, or persistence capability is added. NEXA-TUTOR-001 remains Baseline Draft, the privacy namespace remains reserved, and Phase 4 remains in progress.

### Phase 4 narrow increment: exact model-request token-capacity validation

ADR-0037 adds a separate synchronous gate for an already-constructed ADR-0022 request and existing ADR-0036 evidence. Unchanged conservative UTF-8 byte validation remains mandatory before exact checked token-capacity validation; the operation invokes neither tokenizer nor provider and existing selection and invocation APIs do not consume the evidence. No concrete tokenizer/provider or integration into selection, invocation, admission, authorization, availability, routing, or provider execution is added. Partial truncation and all existing privacy, inference, networking, recovery, semantic-validation, async/streaming, telemetry, and persistence deferrals remain. NEXA-TUTOR-001 remains Baseline Draft, the privacy namespace remains reserved, the known ingestion/context inconsistency is preserved, and Phase 4 remains in progress.
