...
Phase 5 is **in progress**. This matrix records the ADR-0051 through ADR-0059 foundations and the Behavior and Tutor Generation control bindings; it does not claim a complete session runtime.
...
| Five-subsystem cancellation binding and subsystem-specific non-cancellable reporting | Behavior and Tutor Generation control are bound; retrieval, speech, and tool execution remain absent | Not implemented |

NEXA-TUTOR-001 and NEXA-ORCH-001 remain Baseline Draft. Behavior and Tutor Generation control have headless bindings. Tutor evidence proves only exact control-request acceptance and completion/join of the owned control task, not that `LanguageModelProvider::generate` or provider work stopped. Retrieval, speech, tool/lab, renderer, concrete provider/network cancellation, persistence, clock, observability, interruption, timeout, retry, and recovery integrations remain absent.
