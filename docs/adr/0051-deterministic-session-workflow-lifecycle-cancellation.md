...
The V1 wire adds an explicit `1.0` protocol field, rejects unknown fields, versions, variants, nil identities, and malformed values, and preserves exact identity association. Errors are closed and content-free. No payload or replay evidence is necessary for this reference-only aggregate.
