...
The composition performs side-effect-free workflow/four-identity, capability, and exact cancellation-preview validation before planning or spawning. It uses the existing planner to obtain the single canonical `Behavior / Cancellable / RequestCancellation` directive, registers one target-aware task, waits cooperatively for its private token, invokes the exact avatar cancellation once, and returns only after exact-plan execution joins that task. The actual report must exactly equal the approved preview and preserve the request identities and cancelled terminal status; divergence terminalizes the operation as a closed failure. Returned immutable evidence contains the observed task invocation, while bounded read-only adapter inspection permits verification of the owned adapter's request log. Identical successful repeats are idempotent, and conflicts, failed executions, and dropped in-flight executions cannot restart adapter work.
