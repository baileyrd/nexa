...
New wire input rejects unknown fields, unsupported versions, malformed values, nil identities, noncanonical order, duplicates, and collections above the five-target bound. Validation-critical fields remain private and construction is validated. `CancellationPropagationError` is a strict, content-free V1 wire value with the stable variants `unsupported_version`, `association_mismatch`, `invalid_plan`, `dependency_failure`, and `acknowledgement_mismatch`; it rejects unknown fields, versions, variants, and malformed values. The zero-sized `CancellationPropagationDependencyError` belongs only to the dependency-facing Rust port boundary and intentionally is not a wire value. The public operation normalizes that dependency failure, including scripted exhaustion, to the versioned `dependency_failure` operation error.
