...
Updated from the verified latest `main` checkpoint at merge commit `e5f5b71f6dfb1e769d25e0bab32e6eb093ff3803` (PR #29). This is a checkpoint, not an additional specification or ADR. The change containing this status update adds the narrow ADR-0027 capability without assuming a future merge SHA. No commits intervene between that checkpoint and this task branch.
...
- Cross-cutting decisions: the [accepted ADRs](adr/), including [ADR-0021](adr/0021-provider-neutral-tutor-response-planning.md) for response planning, [ADR-0022](adr/0022-provider-neutral-model-invocation.md) for model invocation, [ADR-0023](adr/0023-deterministic-provider-neutral-prompt-compilation.md) for prompt compilation, [ADR-0024](adr/0024-provider-neutral-model-output-admission.md) for model-output admission, [ADR-0025](adr/0025-single-attempt-provider-neutral-invocation-admission.md) for single-attempt invocation-to-admission composition, [ADR-0026](adr/0026-provider-neutral-in-memory-model-registry.md) for provider-neutral registry mechanics, and [ADR-0027](adr/0027-deterministic-provider-neutral-model-selection.md) for static deterministic single-model selection.
...
**Verified roadmap fact:** Phase 4 is the active phase. Provider-neutral invocation, deterministic prompt compilation, structural output admission, ADR-0025 single-attempt invocation-to-admission composition, ADR-0026 immutable registry mechanics, and ADR-0027 static deterministic single-model selection are implemented. Actual inference/concrete providers, dynamic availability/latency/cost/task-complexity routing, automatic local-first routing, fallback, provider tokenization, privacy filtering/remote-provider authorization, semantic validation, retry/repair/regeneration, async/streaming, networking, and persistence remain separate incomplete capability areas. ADR-0024 permits only candidate sections to enter ADR-0021 under trusted caller authority, ADR-0025 composes that admission with exactly one explicitly supplied provider invocation after shared preflight, ADR-0026 does not integrate lookup into that operation, and ADR-0027 does not integrate selection into it.
...
2. Confirm PR #29 / `e5f5b71f6dfb1e769d25e0bab32e6eb093ff3803` is the verified predecessor on `main`, then inspect intervening merges and read ADR-0027 after ADRs 0021–0026.
3. Choose one evidence-backed incomplete Phase 4 increment after ADR-0027's implemented static selection, without assuming this task's eventual merge SHA; state its exclusions and identify specification/ADR/traceability impact before coding.
