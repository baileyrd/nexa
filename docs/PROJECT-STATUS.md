...
Updated from the verified predecessor checkpoint at merge commit `3924b6b25dcc1e47ebdace8324f936844dacf65c` (PR #28). This is a checkpoint, not an additional specification or ADR. The change containing this status update adds the narrow ADR-0026 capability without assuming a future merge SHA. No commits intervene between that checkpoint and this task branch.
...
- Cross-cutting decisions: the [accepted ADRs](adr/), including [ADR-0021](adr/0021-provider-neutral-tutor-response-planning.md) for response planning, [ADR-0022](adr/0022-provider-neutral-model-invocation.md) for model invocation, [ADR-0023](adr/0023-deterministic-provider-neutral-prompt-compilation.md) for prompt compilation, [ADR-0024](adr/0024-provider-neutral-model-output-admission.md) for model-output admission, [ADR-0025](adr/0025-single-attempt-provider-neutral-invocation-admission.md) for single-attempt invocation-to-admission composition, and [ADR-0026](adr/0026-provider-neutral-in-memory-model-registry.md) for provider-neutral registry mechanics.
...
**Verified roadmap fact:** Phase 4 is the active phase. Provider-neutral invocation, deterministic prompt compilation, structural output admission, ADR-0025 single-attempt invocation-to-admission composition, and ADR-0026 immutable registry mechanics are implemented. Actual inference/concrete providers, provider selection/routing/local-first preference/fallback, provider tokenization, privacy filtering/remote-provider authorization, semantic validation, retry/repair/regeneration, async/streaming, networking, and persistence remain separate incomplete capability areas. ADR-0024 permits only candidate sections to enter ADR-0021 under trusted caller authority, ADR-0025 composes that admission with exactly one explicitly supplied provider invocation after shared preflight, and ADR-0026 does not integrate lookup into that operation.
...
2. Confirm PR #28 / `3924b6b25dcc1e47ebdace8324f936844dacf65c` is the verified predecessor on `main`, then inspect intervening merges and read ADR-0026 after ADRs 0021–0025.
3. Choose one evidence-backed incomplete Phase 4 increment after ADR-0026's implemented registry mechanics, without assuming this task's eventual merge SHA; state its exclusions and identify specification/ADR/traceability impact before coding.
