...
/// Closed failures for filtered authorized remote selection followed by
/// usage-validated exact tokenization, invocation, and admission.
#[derive(Clone, Copy, Debug, Error, Eq, PartialEq)]
pub enum FilteredAuthorizedAvailableRemoteUsageValidatedTokenizedInvocationAdmissionError {
    #[error("filtered authorized available remote model selection failed")]
    FilteredSelection(FilteredRemoteSelectionError),
    #[error(
        "selected filtered authorized available remote model usage-validated tokenized invocation or admission failed"
    )]
    UsageValidatedTokenizedInvocationAdmission(UsageValidatedTokenizedInvocationAdmissionError),
}
...
/// Selects through ADR-0034, then validates reported usage between exact tokenization/invocation
/// of the exact filtered input and strict admission.
#[allow(clippy::too_many_arguments)]
pub fn select_filtered_authorized_available_remote_model_tokenize_invoke_validate_reported_usage_and_admit(
    registry: &ModelRegistry,
    invocation_id: ModelInvocationId,
    requirements: &ModelSelectionRequirements,
    availability: &ModelAvailabilitySnapshot,
    ...
    TokenizedInvocationAdmissionResult,
    FilteredAuthorizedAvailableRemoteUsageValidatedTokenizedInvocationAdmissionError,
> {
    let selected = select_filtered_authorized_available_remote_model(
        registry,
        requirements,
    ...
    .map_err(
        FilteredAuthorizedAvailableRemoteUsageValidatedTokenizedInvocationAdmissionError::FilteredSelection,
    )?;
    let compilation = &filtered_result.filtered_compilation;
    let request = request_for_selected(
        invocation_id,
    ...
    .map_err(
        FilteredAuthorizedAvailableRemoteUsageValidatedTokenizedInvocationAdmissionError::UsageValidatedTokenizedInvocationAdmission,
    )
}
